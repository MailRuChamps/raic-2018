...
module MyStrategy = 
    ...
    let private EPS = 0.1f


    let defenseAct (args : ActData) : Action =
        let me, rules, game, _ = args
        let ball, arena = game.ball, rules.arena
        let target_pos_z = -(arena.depth / 2.f) + arena.bottom_radius

        let interception = 
            if ball.velocity_z > -EPS then 
                Vector3(0.f, 0.f, target_pos_z)
            else
                let t = (target_pos_z - ball.z) / ball.velocity_z;
                let x = ball.x + ball.velocity_x * t;
                Vector3(x, 0.f, target_pos_z)

        let target_velocity = Vector3(interception.X - me.x, 0.f,  interception.Z - me.z) * ROBOT_MAX_GROUND_SPEED
        ...
        let jump = delta.Length() < (BALL_RADIUS + ROBOT_MAX_RADIUS) && me.y < ball.y
        let jump_speed = if jump then ROBOT_MAX_JUMP_SPEED else 0.f   
    ...
    let attackEntry (me, arena, ball) : Action option = 
        ...
        let inSim (pos : Vector3, time : float32) = 
            let pos2d = Vector2(pos.X, pos.Z)
            let delta = pos2d - me_pos2d
            let need_speed = delta.Length() / time
            let mx_speed = ROBOT_MAX_GROUND_SPEED
            if 0.5f * mx_speed < need_speed && need_speed < mx_speed then
                let velocity_target = Vector2.Normalize(delta) * need_speed
        ...
        let nextState (pos : Vector3, time : float32) = (pos + ball_pos_delta, time + 0.1f)

        let notInArena (pos : Vector3) = 
            abs(pos.X) > arena.width / 2.f 
            || abs(pos.Z) > arena.depth / 2.f

        simulationLinear inSim nextState (fun (pos,_) -> notInArena pos) (ball_pos, 0.f) 100
